...
Wumpus::Wumpus(Map & map)
{
	StrategyIndex = 0;
	//default spritesheet
	spritesheet[0] = SDL_LoadBMP("img/wumpus.bmp");
	spritesheet[1] = SDL_LoadBMP("img/wumpus.bmp");
	spritesheet[2] = SDL_LoadBMP("img/wumpus.bmp");
	//default initial sprite
	currentsprite = SDL_LoadBMP("img/wumpus.bmp");
	//setup axis
	x = 0;
	y = 0;
...
Wumpus::Wumpus(SDL_Surface * _spritesheet[], Map & map)
{
	StrategyIndex = 0;
...
Wumpus::Update(Map & map)
{
	...
	map.ChangeCell(LastCell[0], LastCell[1], NULL);

	//and we update his position
	map.ChangeCell(x, y, currentsprite);

}
int Wumpus::AI_Verification(Map & map,SDL_Surface * surf){
...
void Wumpus::wantToMove(Map & map,int ch)
{
	//we record LastCell
	LastCell[0] = x;
	LastCell[1] = y;
...
SDL_Surface * Wumpus::getCurrentSprite()
{
...
void Wumpus::setCurrentSprite(SDL_Surface * _new)
{
	currentsprite = _new;
...
bool Wumpus::elaborateStrategy(Map & map)
{
	...
	std::cout << "|V| = " << map.getxDim() * map.getyDim() << std::endl;
	return true;
